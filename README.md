...
Damage calculator for all generations of Pokémon cartridge battling.
...
The `@smgoon/calc` package powers the UI, providing a programmatic interface for computing damage
...
```html
<script src="https://unpkg.com/@smogon/calc/data"></script> <!-- or @pkmn/data -->
...
`@smogon/calc` comes package with all of the data required for damage calculation - by default it
...
In some advanced use cases, you may which to use a different data layer with the calculator. The
...
first must install dependencies by running `npm install`. This should create a `node_modules/`
folder under the root directory and under `calc/` - if there is no `node_modules/` folder under
`calc/`, run `npm install` there as well:

```sh
$ npm install
$ cd calc && npm install # should be unnecessary
```
...
```sh
$ node build
$ open dist/index.html # open works on macOS, simply double-clicking the file on Windows works
```

If you make changes to anything in `calc/`, you must run `node build` from the top level to compile
the files and copy them into `dist/` again. If you make changes to the HTML or JavaScript in `src/`
you must run `node build view` before the changes will become visible in your browser (`node build`
also works, but is slower as it also will compile `calc/` as well, which is unnecessary if you did
not make any changes to that directory).

Before opening up a Pull Request, pelase ensure `npm test` passes:
