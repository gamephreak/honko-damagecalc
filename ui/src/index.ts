import {BattlePokemonSpritesBW as BW} from './data/bw';
import {BattlePokemonSprites as XY} from './data/xy';
...
export const Sprites = new class {
  forBattle(pokemon: string, options: {
    gen?: number, back?: boolean, shiny?: boolean, gender?: Gender, noScale?: boolean, noAnim?: boolean,
  } = {gen: 6}) {
    ...
    let facing = options.back ? 'back' : 'front';
    if (data[`${facing}f`] && options.gender === 'F') facing += 'f';

    let name = data.spriteid;
    if (!noAnim && data[facing] && genNum >= 5) {
      if (facing.slice(-1) === 'f') name += '-f';
      dir = `${gen}ani${dir}`;

      spriteData.w = data[facing].w;
      spriteData.h = data[facing].h;
  ...
  forTeambuilder(pokemon: string, options: {
    gen?: number, shiny?: boolean, gender?: Gender
  } = {gen: 6}) {
    if (!pokemon) return '';
    const data = getData(pokemon);
    const id = data.id;
...
export const Icons = new class {
  getPokemon(pokemon: string, options: {gender?: Gender, left?: boolean} = {}) {
    ...
    } else if (options.gender === 'F' && ['unfezant', 'frillish', 'jellicent', 'meowstic', 'pyroar'].includes(id)) {
  ...
  getType(type: string) {
    return !type ? '' : `<img src="${URL}sprites/types/${type}.png" alt="${type}" height="14" width="32" />`;
  }
};
...
function getData(pokemon: string, bw?: boolean): PokemonData {
  ...
  if (baseid !== id) data.formeid = `-${toID(forme)}`;
